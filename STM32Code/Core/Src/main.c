  ...
  /* USER CODE BEGIN StartServoControlTask */
	...
	/*----------------Animation: Head Nodding---------------------------------------*/
				...
				case 8:	{	
					...
					//allServoLegSpin(pca9685_leg,200,leftLeg.calibrationAngle, rightLeg.calibrationAngle,0);
				break;
				}	
				case 9: {
  ...
  /* USER CODE BEGIN StartHandServoControlTask */
		...
		for(;;)
		{
			switch(state){
				...
				case 8:
					/*------------calibration-------------------------------------------------------------*/
						allServoHandSpin(pca9685_hand,200,leftHand.calibrationAngle, rightHand.calibrationAngle,0);
				break;
				case 9:
					/*----------------loading default values ​​from the server via UART when the hut starts working----------------------------*/
					osMutexAcquire(UARTDataMutexHandle, osWaitForever);
