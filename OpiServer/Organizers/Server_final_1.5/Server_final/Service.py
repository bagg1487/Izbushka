...
class Service:
    ...
    def __init__(self):
        ...
        self.speach_rec = Speaker()  # Объект для распознавания речи
    ...
    def game_mode(self, command):
        ...
        if not 'кнб' in command:
            # Произнести предложение о начале игры в камень-ножницы-бумага или виселицу
            self.speach_rec.speak("Давайте! скажите камень-ножницы-бумага!")
    ...
    def time_now(self):
        ...
        if self.os_name == "Windows":
            ...
            dt = datetime.datetime.now().strftime("%H %M")  # получаем текущее время
        self.speach_rec.speak('Сейчас' + dt)  # озвучиваем
    ...
    def calculate(self, str1):
        ...
        try:
            ...
            if operator == '+':
                ...
                result = None
            # если что-то пошло не так, просим повторить, иначе озвучиваем ответ
            if result is None:
                self.speach_rec.speak('Я не поняла, повторите')  # озвучиваем
            else:
                res = 'Результат ' + str(result)
                self.speach_rec.speak(res)  # озвучиваем
        except:
            self.speach_rec.speak('Я не поняла, повторите')
    ...
    def wikisearch(self, order):
        ...
        print(sentence)
        self.speach_rec.speak('Ищу...')
        results = wikipedia.summary([sentence], sentences=1)
        self.speach_rec.speak("В википедии написано")
        print(results)
        self.speach_rec.speak(results)
    ...
    def joke(self):
        ...
        # озвучиваем шутку
        self.speach_rec.speak(elements[x])
