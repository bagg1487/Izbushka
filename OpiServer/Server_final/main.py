...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        try:
            ...
            prs_path = os.path.join(self.parent_dir, "PRS")
        ...
        # Таймер для обновления видео с камеры
        ...
        # Перехват сигнала закрытия окна
        self.setAttribute(Qt.WA_DeleteOnClose)
        self.destroyed.connect(self.cleanup_on_exit)
    ...
    def cleanup_on_exit(self):
        ...
        print("Очистка ресурсов перед выходом...")

        # Остановка всех таймеров
        if hasattr(self, 'timer'):
            self.timer.stop()
        if hasattr(self, 'timer_interface'):
            self.timer_interface.stop()
        if hasattr(self, 'timer_hold_3'):
            self.timer_hold_3.stop()
        if hasattr(self, 'timer_hold_10'):
        ...
        # Выключение сервера и освобождение аудиоресурсов
        if hasattr(self, 'server') and self.server:
            # Остановка голосового ассистента
            if hasattr(self.server, 'service') and hasattr(self.server.service, 'speach_rec'):
                try:
                    if hasattr(self.server.service.speach_rec, 'stop'):
                        self.server.service.speach_rec.stop()
                    # Остановка распознавания
                    if hasattr(self.server, 'service') and hasattr(self.server.service, 'recognizer'):
                        self.server.service.recognizer.stop()
                except Exception as e:
                    print(f"Ошибка при остановке аудио: {e}")

            # Остановка музыки
            if hasattr(self.server, 'service') and hasattr(self.server.service, 'music_player'):
                try:
                    self.server.service.music_player.stop()
                except:
                    pass

            # Остановка радио
            if hasattr(self.server, 'service') and hasattr(self.server.service, 'radio_player'):
                try:
                    self.server.service.radio_player.pause()
                except:
                    pass

            # Остановка видео
            if hasattr(self.server, 'service') and hasattr(self.server.service, 'video_player'):
                try:
                    if hasattr(self.server.service.video_player, 'stop'):
                        self.server.service.video_player.stop()
                except:
                    pass

            # Выключение TCP сервера
            try:
                self.server.turn_off_server()
            except:
                pass

            # Остановка UART соединения
            if hasattr(self.server, 'uart') and hasattr(self.server.uart, 'serial_close'):
                try:
                    self.server.uart.serial_close()
                except:
                    pass
        ...
        # Принудительная очистка ссылок для GC
        time.sleep(0.3)
    ...
    def next_gif(self):
        """Логика выбора следующей эмоции"""
        ...
        if self.server.service.music_player.player_active_trig:
            if self.server.service.music_player.end_track_trig:
                ...
                self.current_index = random.randint(3, 4) if len(self.gifs) > 4 else 0
