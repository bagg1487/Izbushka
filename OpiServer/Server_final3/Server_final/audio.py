...
# Добавьте эту строку в начало файла для отключения PulseAudio
...
sudo apt install alsa-utils
    ...
    def _tts_worker(self):
        """
        Исправленный TTS worker, использующий espeak с ALSA напрямую
        """
        while self._tts_running:
            ...
            try:
                with self._tts_lock:
                    ...
                    # Используем espeak с выводом в ALSA через пайплайн
                    # espeak генерирует аудио -> aplay воспроизводит через ALSA
                    try:
                        # Создаем пайплайн: espeak -> aplay
                        espeak_cmd = [
                            'espeak',
                            '-v', 'ru',            # русский голос
                            '-s', '150',           # скорость речи
                        ...
                        aplay_cmd = [
                            'aplay',
                            '-D', 'default',       # устройство ALSA по умолчанию
                            '-q'                   # тихий режим
                        ]
                        
                        # Запускаем espeak и передаем вывод в aplay
                        ...
                        # Резервный вариант: просто espeak без aplay
                        print(f"Прямой пайплайн не сработал: {e}, использую простой espeak")
                        subprocess.run(
                            ['espeak', '-v', 'ru', text],
                            stdout=subprocess.DEVNULL,
                            stderr=subprocess.DEVNULL
